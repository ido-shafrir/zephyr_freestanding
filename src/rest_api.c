 ...
 * the endpoint registry, including path, method, and description.
 */
...
HTTP_RESOURCE_DEFINE(api_help_res, rest_api_svc, "/api/help", &api_help_detail);
 ...
 * the endpoint registry, with clickable links and descriptions.
 */
                         ...
                         void *user_data)
{
    if (status == HTTP_SERVER_REQUEST_DATA_FINAL) {
        ...
        int off = snprintf(index_buf, sizeof(index_buf),
                           "<!DOCTYPE html><html><head><title>REST API</title></head><body>"
                           "<h1>Available Endpoints</h1><ul>");
...
HTTP_RESOURCE_DEFINE(index_res, rest_api_svc, "/index", &index_detail);
