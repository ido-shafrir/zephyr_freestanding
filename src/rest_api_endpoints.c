...
/* ==========================================================================
 * Endpoint: GET /api/device/<name>/info
 *
 * Demonstrates a wildcard path endpoint. Registered as "/api/device/*/info"
 * — the handler parses the <name> segment from the URL itself.
 * ========================================================================== */

/** @brief Scratch buffer for /api/device response. */
static char device_buf[256];

/**
 * @brief Handler for GET /api/device/<name>/info.
 *
 * Extracts the device name from the URL path and returns:
 *   {"result": "<name>"}
 *
 * Expects the URL to be exactly /api/device/<name>/info.
                                ...
                                void *user_data)
{
    if (status == HTTP_SERVER_REQUEST_DATA_FINAL) {
        ...
        /* Expected format: /api/device/<name>/info
         * Skip the leading "/api/device/" prefix */
        ...
        /* Find the device name between "/api/device/" and "/info" */
        ...
        if (name_end == NULL || name_end <= name_start) {
            const char err[] = "{\"error\":\"URL must be /api/device/<name>/info\"}";
            send_json(response_ctx, HTTP_400_BAD_REQUEST,
                      err, sizeof(err) - 1);
            return 0;
        }
        ...
        int off = snprintf(device_buf, sizeof(device_buf),
                           "{\"result\":\"%.*s\"}",
                           (int)name_len, name_start);

        send_json(response_ctx, HTTP_200_OK, device_buf, off);
    }

    return 0;
}

static struct http_resource_detail_dynamic api_device_detail = {
    .common = {
        .type = HTTP_RESOURCE_TYPE_DYNAMIC,
...
HTTP_RESOURCE_DEFINE(api_device_res, rest_api_svc, "/api/device/*/info", &api_device_detail);

ENDPOINT_ENTRY_DEFINE(ep_device_info, "/api/device/{name}/info", "GET",
    "Returns {\"result\": \"{name}\"}.");
